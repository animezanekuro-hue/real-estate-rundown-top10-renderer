...
function titleHtml(title, goldWord) {
  ...
  const escapedGold = safeGold.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const pattern = new RegExp(`(${escapedGold})`, 'i');
...
function normalizePayload(body = {}) {
  ...
  return {
    ...
    items: items.map((item, idx) => ({
      name: item.name || item.city || item.market || item.location || `Item ${idx + 1}`,
      sub: item.sub || item.state || item.subtitle || item.description || '',
  ...
  const rows = p.items.map((item, i) => `
    <div class="rr-row">
      ...
      <div class="rr-icon-circle">
        <i class="ti ${icons[i]}" aria-hidden="true"></i>
      </div>
      <div class="rr-city">
  ...
  <style>
    *, *::before, *::after {
    ...
    .rr-hero-grid {
      ...
      inset: 0;
      background-image:
        linear-gradient(rgba(184, 148, 60, 0.07) 1px, transparent 1px),
        linear-gradient(90deg, rgba(184, 148, 60, 0.07) 1px, transparent 1px);
    ...
    .rr-hero-glow {
      ...
      width: 440px;
      height: 440px;
      background: radial-gradient(circle, rgba(184, 148, 60, 0.2) 0%, transparent 70%);
      pointer-events: none;
    }
    ...
    .rr-badge-logo-text span:first-child {
      font-family: 'Oswald', Arial, sans-serif;
      font-size: 22px;
      font-weight: 700;
      color: #ffffff;
    ...
    .rr-badge-logo-text span:last-child {
      font-size: 18px;
      color: #b8943c;
      letter-spacing: 3px;
      text-transform: uppercase;
    ...
    .rr-row {
      ...
      border-radius: 16px;
      padding: 18px 24px;
      border: 2px solid rgba(184, 148, 60, 0.12);
      gap: 20px;
    }
    ...
    .rr-icon-circle {
      width: 60px;
      height: 60px;
      border: 3px solid rgba(184, 148, 60, 0.35);
      border-radius: 50%;
      display: flex;
      align-items: center;
      justify-content: center;
    ...
    .rr-footer {
      ...
      border-top: 2px solid rgba(184, 148, 60, 0.12);
      position: absolute;
      left: 0;
      right: 0;
      bottom: 0;
...
<body>
  <div class="rr-post" id="rr-post">
    <div class="rr-hero">
      <div class="rr-hero-grid"></div>
      <div class="rr-hero-glow"></div>

      <div class="rr-badge-logo">
        <div class="rr-badge-logo-icon">RE</div>
        <div class="rr-badge-logo-text">
          <span>Real Estate</span>
          <span>— Rundown —</span>
        </div>
      </div>

      <div class="rr-pill">${escapeHtml(p.pill)}</div>

      <div class="rr-headline-block">
        <div class="rr-eyebrow">${escapeHtml(p.handle)}</div>
        <div class="rr-title">${titleHtml(p.title, p.goldWord)}</div>
      </div>
    </div>
    ...
    <div class="rr-footer">
      <div class="rr-footer-handle">
        <i class="ti ti-home" aria-hidden="true"></i>
        ${escapeHtml(p.handle)}
      </div>
      <div class="rr-footer-source">${escapeHtml(p.source)}</div>
    </div>
  </div>
</body>
</html>`;
