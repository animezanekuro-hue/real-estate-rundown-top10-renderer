...
async function getBrowser() {
  // PATCH V4: back to the user's known-working Render pattern.
  // Uses pinned puppeteer-core + @sparticuz/chromium versions.
  const chrome = chromium.default || chromium;

  return puppeteer.launch({
    args: [...chrome.args, '--no-sandbox', '--disable-setuid-sandbox'],
    defaultViewport: chrome.defaultViewport || { width: 1080, height: 1080 },
    executablePath: await chrome.executablePath(),
    headless: chrome.headless
  });
}

app.get('/', (req, res) => res.send('Real Estate Rundown Top 10 renderer is running.'));

app.get('/health', (req, res) => res.json({ ok: true, service: 'real-estate-rundown-top10-renderer', patch: 'v4-working-style-sparticuz-pinned' }));
